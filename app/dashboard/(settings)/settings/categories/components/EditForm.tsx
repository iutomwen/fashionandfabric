...
import { updateCategoryById } from "../actions";
...
export default function EditForm({
  ...
  const form = useForm<z.infer<typeof FormSchema>>({
    ...
    defaultValues: {
      name: data?.name,
      icon: data?.icon || "",
      has_options: (data?.has_options as boolean) ? "1" : "0",
    },
  });
  ...
  function onSubmit(values: z.infer<typeof FormSchema>) {
    ...
    startTransition(async () => {
      const result = await updateCategoryById(data?.id, updateData);
      ...
      toast({
        ...
        description: `Category ${updateData.name} has been updated.`,
      });
    });
